...
require ROOT . '/src/Services/Minify.php';
...
$minify = new \Jet\Services\Minify();
$cache_time = $minify->get_cache_time();
...
if (substr($query, - strlen($ext)) !== $ext) {
    $send_301($cache_time['path'] . $cache_time['dir'] . '/' . $query . '&z=' . $ext);
}

// fix the cache dir in the URL
if ($cache_time['dir'] !== $requested_cache_dir) {
    $send_301($cache_time['path'] . $cache_time['dir'] . '/' . $query);
}

$content = $app->minify->combine($sources);

// save and send file
$file = ROOT . $minify->cache_path . $requested_cache_dir . '/' . $query;
